        ...
        for name in "saml_response_failed.xml", "saml_response.xml":
            with self.subTest(name=name):
                ...
                view.request = self.factory.post(
                    self.url, {"SAMLResponse": b64encode(saml_response_xml).decode("ascii")}
                )
                saml_response = view.get_saml_response(None, None, None)
                self.assertXMLEqual(dump_xml(saml_response.document).decode("utf-8"), saml_response_xml.decode("utf-8"))
    ...
    def test_get_saml_response_fix_failure_response(self):
        ...
        for name in "saml_response_failed_nia.xml", "saml_response_failed_nia_decrypted.xml":
            with self.subTest(name=name):
                ...
                saml_response = view.get_saml_response(None, None, None)
                self.assertXMLEqual(
                    dump_xml(saml_response.document).decode("utf-8"), saml_response_xml_fixed.decode("utf-8")
                )
