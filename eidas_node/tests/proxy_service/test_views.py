...
class TestProxyServiceRequestView(IgniteMockMixin, SimpleTestCase):
    ...
    def test_create_saml_request(self):
        ...
        self.assertEqual(
            root.find(".//{}".format(Q_NAMES["saml2:Issuer"])).text, "https://test.example.net/saml/idp.xml"
        )
    ...
    def test_post_success(self):
        ...
        self.assertEqual(
            response.context["identity_provider_endpoint"], "https://test.example.net/identity-provider-endpoint"
        )
        ...
        self.assertIn(
            ...
            "https://test.example.net/saml/idp.xml</saml2:Issuer>",
            saml_request_xml,
        )
        ...
        self.assertContains(
            response, '<form class="auto-submit" action="https://test.example.net/identity-provider-endpoint"'
        )
        self.assertContains(
            response, '<input type="hidden" name="SAMLRequest" value="{}"'.format(response.context["saml_request"])
        )
    ...
    def test_post_failure(self):
        ...
        self.assertNotIn(b"https://example.net/identity-provider-endpoint", response.content)
        self.assertContains(response, "An error occurred during processing of eIDAS Node request.", status_code=400)
    ...
    @freeze_time("2017-12-11 14:12:05")
    @override_settings(PROXY_SERVICE_TRANSIENT_NAME_ID_FALLBACK=True, PROXY_SERVICE_AUXILIARY_STORAGE=AUXILIARY_STORAGE)
    ...
    @freeze_time("2017-12-11 14:12:05")
    @override_settings(PROXY_SERVICE_TRACK_COUNTRY_CODE=True, PROXY_SERVICE_AUXILIARY_STORAGE=AUXILIARY_STORAGE)
    def test_post_remember_country_codes(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                call.connect("test.example.net", 1234),
                call.get_cache("aux-cache"),
                call.get_cache().put("aux-test-light-request-id", '{"citizen_country": "CA", "origin_country": "CA"}'),
            ],
        )
...
class TestIdentityProviderResponseView(IgniteMockMixin, SimpleTestCase):
    ...
    def test_get_saml_response_no_data(self):
        ...
        view.request = self.factory.post(self.url)
        self.assertRaises(ParseError, view.get_saml_response, None, None, None)
    ...
    def test_get_saml_response_encrypted(self):
        ...
        view.request = self.factory.post(
            self.url, {"SAMLResponse": b64encode(saml_response_xml).decode("ascii"), "RelayState": "relay123"}
        )
        saml_response = view.get_saml_response(KEY_SOURCE, [KEY_LOCATION], None)
    ...
    def test_get_saml_response_signed(self):
        ...
        saml_response = view.get_saml_response(None, None, [CERT_FILE])
        self.assertEqual(saml_response.relay_state, "relay123")
    ...
    def test_get_saml_response_signed_multi_cert(self):
        with cast(TextIO, (DATA_DIR / "signed_response_and_assertion.xml").open("r")) as f:
            tree = parse_xml(f.read())
        remove_extra_xml_whitespace(tree)
        saml_response_encoded = b64encode(dump_xml(tree, pretty_print=False)).decode("ascii")
        ...
        view = IdentityProviderResponseView()
        view.request = self.factory.post(self.url, {"SAMLResponse": saml_response_encoded, "RelayState": "relay123"})
        saml_response = view.get_saml_response(None, None, [WRONG_CERT_FILE, CERT_FILE])
        self.assertEqual(saml_response.relay_state, "relay123")
    ...
    def test_get_saml_response_signed_and_encrypted(self):
        ...
        view = IdentityProviderResponseView()
        view.request = self.factory.post(self.url, {"SAMLResponse": saml_response_encoded, "RelayState": "relay123"})
        saml_response = view.get_saml_response(KEY_SOURCE, [KEY_LOCATION], [NIA_CERT_FILE])
        self.assertEqual(saml_response.relay_state, "relay123")
    ...
    def test_get_saml_response_signed_and_encrypted_wrong_key(self):
        with cast(TextIO, (DATA_DIR / "nia_test_response.xml").open("r")) as f:
            tree = parse_xml(f.read())
        remove_extra_xml_whitespace(tree)
        saml_response_encoded = b64encode(dump_xml(tree, pretty_print=False)).decode("ascii")
        view = IdentityProviderResponseView()
        view.request = self.factory.post(self.url, {"SAMLResponse": saml_response_encoded, "RelayState": "relay123"})
        self.assertRaises(SecurityError, view.get_saml_response, KEY_SOURCE, [WRONG_KEY_LOCATION], [NIA_CERT_FILE])

    def test_get_saml_response_signed_and_encrypted_multi_key(self):
        with cast(TextIO, (DATA_DIR / "nia_test_response.xml").open("r")) as f:
            tree = parse_xml(f.read())
        remove_extra_xml_whitespace(tree)
        saml_response_encoded = b64encode(dump_xml(tree, pretty_print=False)).decode("ascii")
        view = IdentityProviderResponseView()
        view.request = self.factory.post(self.url, {"SAMLResponse": saml_response_encoded, "RelayState": "relay123"})
        saml_response = view.get_saml_response(KEY_SOURCE, [WRONG_KEY_LOCATION, KEY_LOCATION], [NIA_CERT_FILE])
        self.assertEqual(saml_response.relay_state, "relay123")
    ...
    def test_get_saml_response_signed_and_encrypted_multi_cert(self):
        with cast(TextIO, (DATA_DIR / "nia_test_response.xml").open("r")) as f:
            tree = parse_xml(f.read())
        remove_extra_xml_whitespace(tree)
        saml_response_encoded = b64encode(dump_xml(tree, pretty_print=False)).decode("ascii")
        view = IdentityProviderResponseView()
        view.request = self.factory.post(self.url, {"SAMLResponse": saml_response_encoded, "RelayState": "relay123"})
        saml_response = view.get_saml_response(KEY_SOURCE, [KEY_LOCATION], [WRONG_CERT_FILE, NIA_CERT_FILE])
        self.assertEqual(saml_response.relay_state, "relay123")
    ...
    @override_settings(PROXY_SERVICE_AUXILIARY_STORAGE=AUXILIARY_STORAGE, PROXY_SERVICE_TRANSIENT_NAME_ID_FALLBACK=True)
    ...
    def test_post_success(self, uuid_mock: MagicMock):
        ...
        response = self.client.post(
            self.url, {"SAMLResponse": b64encode(saml_request_xml).decode("ascii"), "RelayState": "relay123"}
        )
        ...
        self.assertContains(
            response, '<form class="auto-submit" action="https://test.example.net/SpecificProxyServiceResponse"'
        )
    ...
    def test_post_failure(self):
        ...
        self.assertContains(
            response, "An error occurred during processing of Identity Provider response.", status_code=400
        )
