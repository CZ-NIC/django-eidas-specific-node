...
class TestDemoServiceProviderRequestView(SimpleTestCase):
    ...
    def test_get(self):
        ...
        self.assertContains(
            response,
            '<button type="submit" name="Request" value="0">Request mandatory attributes, persistent name id</button>',
        )
        self.assertContains(
            response,
            '<button type="submit" name="Request" value="2">Request mandatory attributes, unspecified name id</button>',
        )
...
class TestDemoServiceProviderResponseView(SimpleTestCase):
    ...
    def test_post_without_relay_state(self):
        ...
        self.assertEqual(
            response.context["saml_response"], "<?xml version='1.0' encoding='utf-8' standalone='yes'?>\n<s/>\n"
        )
        self.assertEqual(response.context["relay_state"], "None")
        self.assertContains(response, "<code>None</code>")
    ...
    def test_post_with_relay_state(self):
        response = self.client.post(
            self.url, {"SAMLResponse": b64encode(b"<s></s>").decode("ascii"), "RelayState": "xyz"}
        )
        self.assertEqual(
            response.context["saml_response"], "<?xml version='1.0' encoding='utf-8' standalone='yes'?>\n<s/>\n"
        )
